...
function TJsonNode.Add(const Name: string; K: TJsonNodeKind = nkObject): TJsonNode;{$IFDEF FPC} overload;{$ENDIF}
begin
  case K of
...
function TJsonNode.Add(const Name: string; B: Boolean): TJsonNode;{$IFDEF FPC} overload;{$ENDIF}
const
...
function TJsonNode.Add(const Name: string; const N: Double): TJsonNode;{$IFDEF FPC} overload;{$ENDIF}
begin
  Result := Add(nkNumber, Name, FloatToStr(N));
end;

function TJsonNode.Add(const Name: string; const S: string): TJsonNode;{$IFDEF FPC} overload;{$ENDIF}
begin
...
begin
  ...
  begin
    ...
    begin
      SetString(S, A, B - A);
      ChildNode {$IFNDEF FPC}:={$ELSE}={$ENDIF} N.Child(S);
