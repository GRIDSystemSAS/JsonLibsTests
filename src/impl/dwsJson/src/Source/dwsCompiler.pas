...
type
   ...
   IdwsDataSymbolFactory = interface
      ...
      function CreateConstSymbol(const name : String; const namePos : TScriptPos; typ : TTypeSymbol;
                                 const data : IDataContext) : TConstSymbol;
...
begin
   ...
   FDataSymbolExprReuse := nil; /// disabled for now TSimpleObjectObjectHash_TDataSymbol_TVarExpr.Create;
         ...
         if not expr.IsConstant then begin
            ...
            if typ is TArraySymbol then begin
               ...
               if expr is TConstExpr then begin
                  Result := factory.CreateConstSymbol(name, constPos, sas, TConstExpr(expr).DataContext);
               end else begin
                  recordData := TDataContext.CreateStandalone(expr.Typ.Size);
                  (expr as TArrayConstantExpr).EvalToDataContext(FExec, recordData, 0);
...
begin
   ...
   case FTok.TestDeleteAny([ttIF, ttCASE, ttFOR, ttWHILE, ttREPEAT, ttBREAK,
      ...
      if (FTok.TestAny([ttBLEFT, ttINHERITED, ttNEW])<>ttNone) or FTok.TestName then begin // !! TestName must be the last !!
         ...
         if locExpr = nil then begin
            FMsgs.AddCompilerStop(hotPos, CPE_InvalidInstruction);
            Exit(nil);
         end;
         locExprClass := locExpr.ClassType;
         if locExprClass.InheritsFrom(TTypedExpr) then begin
            if FTok.TestAny([ttLESS_LESS, ttGTR_GTR]) <> ttNone then begin
         ...
         try
            ...
            if token <> ttNone then begin
               ...
               if not locExprClass.InheritsFrom(TDataExpr) then begin
               ...
               if locExprClass.InheritsFrom(TAssignExpr) then
                  ...
                  if FMsgs.Count = msgsCount then   // avoid hint on expression with issues
                     FMsgs.AddCompilerHint(hotPos, CPE_ConstantInstruction);
               end else if locExprClass.InheritsFrom(TNullExpr) then begin
                  Result := locExpr;
               end else if (FPendingSetterValueExpr <> nil) and locExpr.InheritsFrom(TTypedExpr) then begin
...
begin
   ...
   if Assigned(sym) then begin
      ...
      if sym is TMethodSymbol then begin
         ...
         varExpr:=TVarExpr.CreateTyped(FCompilerContext, FTok.HotPos, methSym.SelfSym);
         try
...
begin
   ...
   try
      ...
      if stValueSymbol in symTaxonomy then begin // symClassType.InheritsFrom(TValueSymbol) then begin

         // value symbols

         if stConstSymbol in symTaxonomy then begin  // symClassType.InheritsFrom(TConstSymbol)

            Result := ReadConstName(namePos, TConstSymbol(sym), IsWrite);

         end else if stDataSymbol in symTaxonomy then begin // symClassType.InheritsFrom(TDataSymbol) then begin
         ...
         end else if symClassType = TFieldSymbol then begin
         ...
         end else if symClassType = TPropertySymbol then begin
         ...
         end else if symClassType = TExternalVarSymbol then begin
         ...
         if baseType is TStructuredTypeSymbol then begin
            ...
            if baseTypeClass = TClassSymbol then begin

               Result := ReadClassSymbolName(TClassSymbol(baseType), isWrite, expecting);

            end else if baseTypeClass = TInterfaceSymbol then begin
         ...
         end else if symClassType.InheritsFrom(TFuncSymbol) then begin

            if symClassType.InheritsFrom(TMethodSymbol) then begin
         ...
         end else if symClassType.InheritsFrom(TEnumerationSymbol) then begin
      ...
      if baseType.IsExternalRooted then begin
         ...
         if operandExpr<>nil then begin
            ...
            if castedExprTyp <> FCompilerContext.TypNil then begin
               ...
               if    (not castedExprTyp.IsClassSymbol)
                  or (
      ...
      if FTok.TestDeleteAnyNamePos(name, namePos) then begin
         ...
         if baseType.InheritsFrom(TStructuredTypeSymbol) then begin
            ...
            if member = nil then begin
               ...
               if memberClassType.InheritsFrom(TMethodSymbol) then begin
               ...
               end else if memberClassType.InheritsFrom(TFieldSymbol) then begin

                  Assert(Result is TTypedExpr);
                  Result := ReadField(FTok.HotPos, nil, TFieldSymbol(member), TTypedExpr(Result));

               end else if memberClassType.InheritsFrom(TPropertySymbol) then begin
...
begin
   ...
   try
      if IsWrite and not FTok.Test(ttDOT) then begin
         ...
         end else if Sym.Typ.IsClassSymbol or (Sym.Typ is TClassOfSymbol) or (Sym.Typ is TConnectorSymbol) then begin
...
begin
   ...
   if (inExpr is TTypedExpr) and (inExpr.ClassType <> TTypeReferenceExpr) then begin
      ...
      if enumSymbol is TEnumerationSymbol then begin
         fromExpr := TConstIntExpr.Create(loopVarNamePos, loopVarExpr.Typ, TEnumerationSymbol(enumSymbol).LowBound);
         toExpr := TConstIntExpr.Create(loopVarNamePos, loopVarExpr.Typ, TEnumerationSymbol(enumSymbol).HighBound);
      end else begin
...
(*
               ...
               if funcExprParamType.Typ.IsOfType(matchParamType.Typ) then begin
                  ...
                  if funcExprParamType.Typ.IsClassSymbol or (funcExprParamType.Typ is TInterfaceSymbol) then begin
                  ...
                  Inc(matchDistance, 128);
                           ...
                           or (    (funcExprParamType.Typ=FCompilerContext.TypNil)
                               and (   matchParamType.Typ.IsClassSymbol
               ...
               if funcExprParamType.IsClassSymbol or (funcExprParamType is TInterfaceSymbol) then begin
         ...
         repeat
            ...
            if lowBound is TTypeReferenceExpr then begin
               ...
               if TTypeReferenceExpr(lowBound).Typ is TBaseBooleanSymbol then begin
                  ...
                  min.Insert0(TConstIntExpr.Create(hotPos, enumSymbol, enumSymbol.LowBound));
                  max.Insert0(TConstIntExpr.Create(hotPos, enumSymbol, enumSymbol.HighBound));
      ...
      emName, emQualifiedName : begin
         ...
         if baseExpr.ClassType=TConstIntExpr then begin
            ...
            if element=nil then begin
               ...
               Result := TConstStringExpr.Create(namePos, FCompilerContext.TypString, element.Name)
            else Result := TConstStringExpr.Create(namePos, FCompilerContext.TypString, element.QualifiedName);
            OrphanAndNil(baseExpr);
...
begin
   ...
   if not sym.IsClassSymbol then begin
      if Assigned(sym) then
   ...
   try
      try
         ...
         if FTok.TestDelete(ttBLEFT) then begin
            ...
            if not typ.IsClassSymbol then begin
               ...
               if (typ=nil) or (expr=nil) then begin
                  ...
                  if not FMsgs.HasErrors then begin
                     ...
                     case typ.Size of
                        ...
                        try
                           exprData.WriteData((expr as TDataExpr).DataPtr[FExec], typ.Size);
                        finally
...
begin
   if FIsExcept and (FTok.TestAny([ttSEMI, ttEND, ttELSE, ttUNTIL, ttFINALLY, ttEXCEPT])<>ttNone) then
      ...
      if not (    exceptObjTyp.IsClassSymbol
   ...
   if FTok.TestDelete(ttALEFT) then begin
      ...
      try
         ...
         if setExpr.Typ is TArraySymbol then begin
            ...
            if setExpr.Typ.IsClassSymbol then begin
      ...
      try
         ...
         if (expr<>nil) and (memberTyp<>nil) then begin
            if expr.Typ.IsOfType(FCompilerContext.TypInteger) and memberTyp.IsOfType(FCompilerContext.TypFloat) then
               ...
               result.WriteData(memberSym.Offset, (expr as TDataExpr).DataPtr[FExec], 0, memberTyp.Size);
            end;
         end;
      finally
         OrphanAndNil(expr);
      ...
      case paramSemantics of
         ...
         if Assigned(defaultExpr) then begin
            if defaultExpr.ClassType = TArrayConstantExpr then begin
               ...
               else TArrayConstantExpr(defaultExpr).EvalToDataContext(FExec, data, 0);
               ...
               paramSym := TParamSymbolWithDefaultValue.Create(
                  curName, paramType,
                  (defaultExpr as TConstExpr).DataContext,
                  paramOptions
               );
            end;
         end else begin
            ...
            repeat
               ...
               if not FTok.TestDelete(ttCOLON) then begin
                  ...
                  try
                     ...
                     if (paramSemantics <> pssConst) and (typ <> nil) and (typ.ClassType = TOpenArraySymbol) then
...
begin
   if (dataSym.ClassType=TClassVarSymbol) and (TClassVarSymbol(dataSym).OwnerSymbol.IsExternal) then
      ...
      if FDataSymbolExprReuse<>nil then begin
         ...
         Result.IncRefCount;
      end else Result:=TVarExpr.CreateTyped(FCompilerContext, aScriptPos, dataSym);
...
begin
  ...
  else Result := TVarParamParentExpr.Create(cNullPos,dataSym)
end;
...
begin
   ...
   else Result := TConstParamParentExpr.Create(cNullPos, dataSym)
end;
...
begin
   ...
   for param in funcSym.Params do begin
      ...
      if not param.Typ.IsClassSymbol then continue;
...
begin
   if FTok.Test(ttALEFT) then begin
      ...
      if Result=nil then
         Result:=TConstExpr.CreateNull(cNullPos, FCompilerContext.TypVariant); // keep compiling
      ...
      case specialKind of
         ...
         skHigh : begin
            if argTyp is TArraySymbol then begin
               ...
               OrphanAndNil(argExpr);
               Result := TConstIntExpr.Create(namePos, argTyp, TEnumerationSymbol(argTyp).HighBound)
         ...
         skLow : begin
            if argTyp is TArraySymbol then begin
               ...
               if argTyp is TEnumerationSymbol then begin
                  Result := TConstIntExpr.Create(namePos, argTyp, TEnumerationSymbol(argTyp).LowBound);
...
begin
   ...
   if coSymbolDictionary in CompilerContext.Options then
      RecordSymbolUse(TDataExpr(argExpr).DataSymbol, argExpr.ScriptPos, [ suWrite ]);
